...
export default function LivePage({ navigate: _navigate }: LivePageProps) {
  ...
  const subscribersRef = useRef<number>(0);
  ...
  // Auto-scroll chat within container only (never scroll the page)
  // biome-ignore lint/correctness/useExhaustiveDependencies: chatContainerRef is a stable ref
  useEffect(() => {
  ...
  // Sync subscribersRef with current subscriber count
  useEffect(() => {
  ...
  const startStreamEngine = useCallback(() => {
    // Subscriber-based viewer growth
    viewerIntervalRef.current = setInterval(() => {
      const subs = subscribersRef.current;
      // Scale growth rate based on subscriber count
      let baseGrowth: number;
      let maxGrowth: number;
      if (subs >= 40_000_000) {
        baseGrowth = 80_000;
        maxGrowth = 300_000;
      ...
      setLiveViewers((prev) => {
        // Natural fluctuation: 90% chance grow, 10% chance slight dip
        const isDip = Math.random() < 0.1;
        if (isDip) {
          const dip = Math.floor(baseGrowth * 0.05 * Math.random());
          return Math.max(0, prev - dip);
        }
        // Viral boost: occasionally spike
        const isSpike = Math.random() < 0.05;
        const multiplier = isSpike ? 3 + Math.random() * 2 : 1;
        const growth = Math.floor(
          (baseGrowth + Math.random() * (maxGrowth - baseGrowth)) * multiplier,
        );
        // Allow viewers to exceed subscriber count (viral streams)
        return prev + growth;
      });
      ...
      setLikes((prevLikes) => {
        ...
        if (subs2 >= 10_000_000)
          likeGrowth = Math.floor(
            baseGrowth * 0.6 * (0.9 + Math.random() * 0.2),
          );
        else
          likeGrowth = Math.floor(
            baseGrowth * 0.6 * (0.8 + Math.random() * 0.4),
          );
        return prevLikes + likeGrowth;
      });
